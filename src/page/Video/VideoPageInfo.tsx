    ...
    return (
        <div className='moe-video-video-page-info'>
            ...
            <div className='moe-video-video-page-count'>
                <span className='select-none'><PlaybackVolumeIcon/>{playCount}</span>
                <span onClick={likeVideo} className='cursor-pointer select-none'
                      style={{backgroundColor: userLiked ? '#b796c0' : ''}}
                >
                     <LikeIcon/>
                    {parseInt(likeCount) + (userLiked ? 1 : 0)}
                </span>
                ...
                >
                    <FavoriteIcon/>{parseInt(favoriteCount) + (userFavorite ? 1 : 0)}
                </span>
                <span className='select-none cursor-pointer' onClick={() => {
                    showModal({
                        title: '举报',
                        content: (<form className='moe-video-video-page-report-modal' ref={formRef}>
                            <p>举报原因</p>
                            <select name='cause'>
                                <option>色情低俗</option>
                                <option>政治敏感</option>
