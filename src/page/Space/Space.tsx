...
import HistoryList, {HistoryListItemProps} from "./HistoryList.tsx";
...
const Space = memo(() => {
    ...
    useEffect(() => {
        ...
        setUploadList([]);
        setHistoryList([]);
        setActiveIndex(pathname.split('/')[2] === 'favor' ? 0 :
            pathname.split('/')[2] === 'bangumi' ? 1 :
                pathname.split('/')[2] === 'upload' ? 2 :
                    pathname.split('/')[2] === 'history' ? 3 :
                        pathname.split('/')[2] === 'setting' ? 4 : 0);
        if (activeIndex === 0) {
                                ...
                                <div className='moe-video-space-page-favor-list-item overflow-hidden' key={index}>
                                    <a href={item.url}>
                                        ...
                                        <div className='flex flex-col justify-between'>
                                            <div
                                                className='moe-video-space-page-favor-list-item-title px-3 pt-1 mb-1 line-clamp-2'>{item.title}</div>
                                        </div>
                                    </a>
                                ...
                                <div className='moe-video-space-page-bangumi-list-item flex py-4 px-2' key={index}>
                                    <a href={item.url}
                                       className='moe-video-space-page-bangumi-list-item-cover mr-4 overflow-hidden'>
                                        <Image lazy width='100%' style={{aspectRatio: '3 / 4', objectFit: 'cover'}}
                                               src={item.cover}/>
                                    </a>
                                    <div className='flex flex-col justify-between overflow-hidden'>
                                        <div>
                                            <div
                                                className='moe-video-space-page-bangumi-list-item-title pb-1 line-clamp-1'>{item.title}</div>
                                            <div className='text-gray-400 text-xs line-clamp-1'>{item.updateTo}</div>
                                            <div
                                                className='text-sm text-gray-550 line-clamp-4 whitespace-break-spaces pt-1 mb-2'>
                                                {item.desc}
                                            </div>
                                            <div
                                                className='text-gray-400 text-xs line-clamp-1 mb-1'>看到 {item.lastWatchedAt}</div>
                                        </div>
